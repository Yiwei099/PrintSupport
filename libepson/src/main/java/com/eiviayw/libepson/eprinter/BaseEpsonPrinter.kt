...
import com.eiviayw.libcommon.base.PrinterInterface
import com.eiviayw.libcommon.bean.Result
import com.eiviayw.libcommon.bean.mission.GraphicMission
import com.eiviayw.libepson.bean.mission.command.CommandMissionParam
import com.eiviayw.libepson.bean.mission.command.DrawerMissionParam
import com.eiviayw.libepson.bean.mission.command.EpsonMission
import com.eiviayw.libepson.bean.mission.command.TextMissionParam
...
abstract class BaseEpsonPrinter(
    ...
    private suspend fun handleMission(mission: BaseMission) {
        when (mission) {
            ...
            is com.eiviayw.libepson.bean.mission.command.EpsonMission -> {
                //指令模式
    ...
    private fun sendDataByCommand(param: com.eiviayw.libepson.bean.mission.command.EpsonMission){
        try{
            mPrinter.beginTransaction()
            param.params.forEach {
                when(it){
                    is com.eiviayw.libepson.bean.mission.command.DrawerMissionParam -> mPrinter.addPulse(Printer.PARAM_DEFAULT,Printer.PARAM_DEFAULT)
                    is com.eiviayw.libepson.bean.mission.command.TextMissionParam -> mPrinter.addText(it.data)
                    is com.eiviayw.libepson.bean.mission.command.CommandMissionParam -> mPrinter.addCommand(it.data)
                    else ->{/* 暂未支持的类型 */}
                }
            }
            mPrinter.sendData(Printer.PARAM_DEFAULT)
