package com.eiviayw.libgprinter.native
...
import com.eiviayw.libcommon.util.BitmapUtils
import com.eiviayw.libcommon.util.SerializationUtils
...
class NativeUsbPrinter(
    ...
    private suspend fun startPrint() {
        getMissionQueue().peekFirst()?.let { param ->
            val result = when (param) {
                ...
                is com.eiviayw.libgprinter.bean.mission.command.GPrinterMission -> {
                    //指令模式
    ...
    private fun sendEscDataByCommandParam(
        param: com.eiviayw.libgprinter.bean.mission.command.GPrinterMission,
        clearCache: Boolean = true
    ): Result {
        if (clearCache) initPrinter()
